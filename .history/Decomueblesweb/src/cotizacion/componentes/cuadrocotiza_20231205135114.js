...
const CuadroCotiza = () => {
  ...
  const handleNext = () => {
    if(validarpaso == 0){setActiveStep((prevActiveStep) => Math.min(prevActiveStep + 1, totalSteps - 1)); alert('Paso' + validarpaso);}
    else{
  ...
  const validarpaso = () => {
    if (activeStep === 0) {
      return telefono.trim() !== '' && nombre.trim() !== '' && descripcion.trim() !== '';
    }
